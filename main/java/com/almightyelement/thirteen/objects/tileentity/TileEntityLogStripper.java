package com.almightyelement.thirteen.objects.tileentity;
...
public class TileEntityLogStripper extends TileEntity implements ITickable
{
	...
	{
		return this.hasCustomName() ? new TextComponentString(this.customName) : new TextComponentTranslation("container.log_stripper");
	}
	...
	public static boolean isBurning(TileEntityLogStripper te) 
	{
	...
	{	
		...
		{
			--this.burnTime;
			BlockLogStripper.setState(true, world, pos);
		}
		...
		else
		{
			...
			{
				ItemStack output = LogStripperRecipes.getInstance().getLogStripperResult(inputs[0], inputs[1]);
				if(!output.isEmpty())
				{
					smelting = output;
					cookTime++;
	...
	private boolean canSmelt() 
	{
		...
		{
			ItemStack result = LogStripperRecipes.getInstance().getLogStripperResult((ItemStack)this.handler.getStackInSlot(0), (ItemStack)this.handler.getStackInSlot(1));	
